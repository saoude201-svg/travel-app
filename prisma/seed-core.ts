...
const HOTEL_PREFIXES = ["The", "Hotel", "Casa", "Villa", "Grand", "Maison", "The Old", "Riad"];
const HOTEL_CORES = [
...
/**
 * Idempotent seed: wipes domain data and recreates a believable catalog plus
 * demo + admin users. Accepts any Prisma client instance so it can run from
 * the CLI (prisma/seed.ts) or a web route (src/app/api/setup/seed).
 */
export async function seedDatabase(
  ...
  const hotelName = (seed: number): string => {
    const r = mulberry32(seed);
    const prefix = r() > 0.35 ? pick(HOTEL_PREFIXES) + " " : "";
  ...
  await db.user.upsert({
    ...
    update: {},
    create: { email: "demo@wanderlust.test", name: "Demo Traveler", passwordHash: demoPassword, role: "USER" },
  });
  await db.user.upsert({
    where: { email: "admin@wanderlust.test" },
    update: { role: "ADMIN" },
    create: { email: "admin@wanderlust.test", name: "Admin", passwordHash: demoPassword, role: "ADMIN" },
  });
  log("  ✓ demo + admin users (password: password123)");
  ...
  for (const [di, d] of destinations.entries()) {
    ...
    for (let h = 0; h < numHotels; h++) {
      ...
      const base =
        Math.round((d.avgDailyBudgetCents * (0.7 + star * 0.28) * between(0.85, 1.25)) / 100) * 100;
      ...
      const hotel = await db.hotel.create({
        data: {
          ...
          images: [img(`${slug}-1`), img(`${slug}-2`), img(`${slug}-3`), img(`${slug}-4`), img(`${slug}-5`)],
          amenities: sample(AMENITIES, intBetween(7, 12)),
      ...
      const templates = sample(ROOM_TEMPLATES, intBetween(3, 5));
      ...
      if (minRoomPrice < base) {
        await db.hotel.update({ where: { id: hotel.id }, data: { basePriceCents: minRoomPrice } });
      }
      ...
      for (let rv = 0; rv < numReviews; rv++) {
        await db.review.create({
          data: {
            ...
            rating: Number(Math.min(10, Math.max(6, guestRating + between(-1.2, 0.8))).toFixed(1)),
            title: pick(REVIEW_TITLES),
            body: pick(REVIEW_BODIES),
            stayDate: new Date(Date.now() - intBetween(20, 400) * 24 * 60 * 60 * 1000),
          },
        });
      }
    }
  ...
  log(`  ✓ ${destinations.length} destinations`);
  log(`  ✓ ${hotelCount} hotels`);
  log(`  ✓ ${roomCount} room types`);
  log("✅ Seed complete.");

  return { destinations: destinations.length, hotels: hotelCount, roomTypes: roomCount };
}
